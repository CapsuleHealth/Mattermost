...
test-server: start-docker prepare-enterprise
	...
	@# MODIFIED 2017-03-02: increasing timeout for api tests
	$(GO) test $(GOFLAGS) -run=$(TESTS) -test.v -test.timeout=900s -covermode=count -coverprofile=capi.out ./api || exit 1
